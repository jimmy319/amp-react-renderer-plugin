...
test('render a valid AMP page with amp-springboard-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-springboard-player'
  const canonical = 'https://amp-springboard-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-springboard-player',
    AppComponent,
    title,
    canonical,
    headComponents
