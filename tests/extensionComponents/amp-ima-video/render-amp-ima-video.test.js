...
test('render a valid AMP page with amp-ima-video component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-ima-video'
  const canonical = 'https://amp-ima-video.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-ima-video',
    AppComponent,
    title,
    canonical,
    headComponents
