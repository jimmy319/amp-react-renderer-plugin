...
test('render a valid AMP page with amp-instagram component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-instagram'
  const canonical = 'https://amp-instagram.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-instagram',
    AppComponent,
    title,
    canonical,
    headComponents
