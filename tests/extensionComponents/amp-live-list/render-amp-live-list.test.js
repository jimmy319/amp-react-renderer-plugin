...
test('render a valid AMP page with amp-live-list component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-live-list'
  const canonical = 'https://amp-live-list-serviceworker.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-live-list',
    AppComponent,
    title,
    canonical,
    headComponents
