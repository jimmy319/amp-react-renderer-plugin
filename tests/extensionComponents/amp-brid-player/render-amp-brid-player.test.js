...
test('render a valid AMP page with amp-brid-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-brid-player'
  const canonical = 'https://amp-brid-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-brid-player',
    AppComponent,
    title,
    canonical,
    headComponents
