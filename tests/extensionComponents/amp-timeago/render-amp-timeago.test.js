...
test('render a valid AMP page with amp-timeago component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-timeago'
  const canonical = 'https://amp-timeago.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-timeago',
    AppComponent,
    title,
    canonical,
    headComponents
