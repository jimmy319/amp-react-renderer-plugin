...
test('render a valid AMP page with amp-nexxtv-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-nexxtv-player'
  const canonical = 'https://amp-nexxtv-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-nexxtv-player',
    AppComponent,
    title,
    canonical,
    headComponents
