...
test('render a valid AMP page with amp-playbuzz component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-playbuzz'
  const canonical = 'https://amp-playbuzz.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-playbuzz',
    AppComponent,
    title,
    canonical,
    headComponents
