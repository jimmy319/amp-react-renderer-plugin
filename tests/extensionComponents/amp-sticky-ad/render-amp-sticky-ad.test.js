...
test('render a valid AMP page with amp-sticky-ad component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-sticky-ad'
  const canonical = 'https://amp-sticky-ad.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-sticky-ad',
    AppComponent,
    title,
    canonical,
    headComponents
