...
test('render a valid AMP page with amp-reach-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-reach-player'
  const canonical = 'https://amp-reach-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-reach-player',
    AppComponent,
    title,
    canonical,
    headComponents
