...
test('render a valid AMP page with amp-jwplayer component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-jwplayer'
  const canonical = 'https://amp-jwplayer.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-jwplayer',
    AppComponent,
    title,
    canonical,
    headComponents
