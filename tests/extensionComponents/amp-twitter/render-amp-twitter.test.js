...
test('render a valid AMP page with amp-twitter component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-twitter'
  const canonical = 'https://amp-twitter.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-twitter',
    AppComponent,
    title,
    canonical,
    headComponents
