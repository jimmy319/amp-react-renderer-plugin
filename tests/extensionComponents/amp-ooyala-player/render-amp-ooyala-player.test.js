...
test('render a valid AMP page with amp-ooyala-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-ooyala-player'
  const canonical = 'https://amp-ooyala-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-ooyala-player',
    AppComponent,
    title,
    canonical,
    headComponents
