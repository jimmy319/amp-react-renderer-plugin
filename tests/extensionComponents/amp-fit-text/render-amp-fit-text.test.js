...
test('render a valid AMP page with amp-fit-text component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-fit-text'
  const canonical = 'https://amp-fit-text.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-fit-text',
    AppComponent,
    title,
    canonical,
    headComponents
