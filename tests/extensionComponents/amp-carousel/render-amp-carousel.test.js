...
test('render a valid AMP page with amp-carousel component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-carousel'
  const canonical = 'https://amp-carousel.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-carousel',
    AppComponent,
    title,
    canonical,
    headComponents
