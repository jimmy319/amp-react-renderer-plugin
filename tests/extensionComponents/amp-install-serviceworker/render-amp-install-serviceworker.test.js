...
test('render a valid AMP page with amp-install-serviceworker component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-install-serviceworker'
  const canonical = 'https://amp-install-serviceworker.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-install-serviceworker',
    AppComponent,
    title,
    canonical,
    headComponents
