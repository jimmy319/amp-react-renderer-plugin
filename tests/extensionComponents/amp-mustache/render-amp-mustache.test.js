...
test('render a valid AMP page with amp-mustache component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-mustache'
  const canonical = 'https://amp-mustache.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-mustache',
    AppComponent,
    title,
    canonical,
    headComponents
