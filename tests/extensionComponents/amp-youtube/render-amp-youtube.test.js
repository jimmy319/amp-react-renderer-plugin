...
test('render a valid AMP page with amp-youtube component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-youtube'
  const canonical = 'https://amp-youtube.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-youtube',
    AppComponent,
    title,
    canonical,
    headComponents
