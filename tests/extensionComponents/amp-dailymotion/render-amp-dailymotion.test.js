...
test('render a valid AMP page with amp-dailymotion component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-dailymotion'
  const canonical = 'https://amp-dailymotion.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-dailymotion',
    AppComponent,
    title,
    canonical,
    headComponents
