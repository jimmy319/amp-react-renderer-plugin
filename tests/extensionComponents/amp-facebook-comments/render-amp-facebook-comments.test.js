...
test('render a valid AMP page with amp-facebook-comments component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-facebook-comments'
  const canonical = 'https://amp-facebook-comments.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-facebook-comments',
    AppComponent,
    title,
    canonical,
    headComponents
