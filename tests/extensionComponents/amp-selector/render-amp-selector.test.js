...
test('render a valid AMP page with amp-selector component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-selector'
  const canonical = 'https://amp-selector.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-selector',
    AppComponent,
    title,
    canonical,
    headComponents
