...
test('render a valid AMP page with amp-sidebar component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-sidebar'
  const canonical = 'https://amp-sidebar.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-sidebar',
    AppComponent,
    title,
    canonical,
    headComponents
