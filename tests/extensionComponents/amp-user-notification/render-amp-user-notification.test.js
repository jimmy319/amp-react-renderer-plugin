...
test('render a valid AMP page with amp-user-notification component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-user-notification'
  const canonical = 'https://amp-user-notification.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-user-notification',
    AppComponent,
    title,
    canonical,
    headComponents
