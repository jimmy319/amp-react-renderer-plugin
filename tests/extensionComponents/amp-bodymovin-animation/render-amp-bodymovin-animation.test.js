...
test('render a valid AMP page with amp-bodymovin-animation component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-bodymovin-animation'
  const canonical = 'https://amp-bodymovin-animation.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-bodymovin-animation',
    AppComponent,
    title,
    canonical,
    headComponents
