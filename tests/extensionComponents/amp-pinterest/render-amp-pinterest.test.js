...
test('render a valid AMP page with amp-pinterest component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-pinterest'
  const canonical = 'https://amp-pinterest.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-pinterest',
    AppComponent,
    title,
    canonical,
    headComponents
