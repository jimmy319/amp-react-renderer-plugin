...
test('render a valid AMP page with amp-date-picker component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-date-picker'
  const canonical = 'https://amp-date-picker.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-date-picker',
    AppComponent,
    title,
    canonical,
    headComponents
