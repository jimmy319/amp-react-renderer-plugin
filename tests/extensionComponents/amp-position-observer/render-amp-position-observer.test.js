...
test('render a valid AMP page with amp-position-observer component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-position-observer'
  const canonical = 'https://amp-position-observer.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-position-observer',
    AppComponent,
    title,
    canonical,
    headComponents
