...
test('render a valid AMP page with amp-image-lightbox component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-image-lightbox'
  const canonical = 'https://amp-image-lightbox.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-image-lightbox',
    AppComponent,
    title,
    canonical,
    headComponents
