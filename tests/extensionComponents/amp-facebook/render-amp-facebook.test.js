...
test('render a valid AMP page with amp-facebook component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-facebook'
  const canonical = 'https://amp-facebook.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-facebook',
    AppComponent,
    title,
    canonical,
    headComponents
