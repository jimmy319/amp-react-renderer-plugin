...
test('render a valid AMP page with amp-fx-flying-carpet component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-fx-flying-carpet'
  const canonical = 'https://amp-fx-flying-carpet.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-fx-flying-carpet',
    AppComponent,
    title,
    canonical,
    headComponents
