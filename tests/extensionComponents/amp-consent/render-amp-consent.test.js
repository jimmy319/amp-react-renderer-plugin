...
test('render a valid AMP page with amp-consent component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-consent'
  const canonical = 'https://amp-consent.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-consent',
    AppComponent,
    title,
    canonical,
    headComponents
