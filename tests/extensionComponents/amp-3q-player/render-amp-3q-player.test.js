...
test('render a valid AMP page with amp-3q-player component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-3q-player'
  const canonical = 'https://amp-3q-player.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-3q-player',
    AppComponent,
    title,
    canonical,
    headComponents
