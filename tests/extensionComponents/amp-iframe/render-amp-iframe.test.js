...
test('render a valid AMP page with amp-iframe component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-iframe'
  const canonical = 'https://amp-iframe.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-iframe',
    AppComponent,
    title,
    canonical,
    headComponents
