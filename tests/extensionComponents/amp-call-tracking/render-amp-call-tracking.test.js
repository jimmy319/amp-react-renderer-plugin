...
test('render a valid AMP page with amp-call-tracking component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-call-tracking'
  const canonical = 'https://amp-call-tracking.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-call-tracking',
    AppComponent,
    title,
    canonical,
    headComponents
