...
test('render a valid AMP page with amp-auto-ads component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-auto-ads'
  const canonical = 'https://amp-auto-ads.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-auto-ads',
    AppComponent,
    title,
    canonical,
    headComponents
