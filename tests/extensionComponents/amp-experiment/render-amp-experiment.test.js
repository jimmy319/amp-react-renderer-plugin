...
test('render a valid AMP page with amp-experiment component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-experiment'
  const canonical = 'https://amp-experiment.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-experiment',
    AppComponent,
    title,
    canonical,
    headComponents
