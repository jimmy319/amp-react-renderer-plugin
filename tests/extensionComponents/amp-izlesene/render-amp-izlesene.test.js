...
test('render a valid AMP page with amp-izlesene component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-izlesene'
  const canonical = 'https://amp-izlesene.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-izlesene',
    AppComponent,
    title,
    canonical,
    headComponents
