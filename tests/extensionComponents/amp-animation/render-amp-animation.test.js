...
test('render a valid AMP page with amp-animation component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-animation'
  const canonical = 'https://amp-animation.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-animation',
    AppComponent,
    title,
    canonical,
    headComponents
