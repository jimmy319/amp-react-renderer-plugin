...
test('render a valid AMP page with amp-mathml component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-mathml'
  const canonical = 'https://amp-mathml.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-mathml',
    AppComponent,
    title,
    canonical,
    headComponents
