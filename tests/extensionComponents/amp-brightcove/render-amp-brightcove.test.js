...
test('render a valid AMP page with amp-brightcove component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-brightcove'
  const canonical = 'https://amp-brightcove.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-brightcove',
    AppComponent,
    title,
    canonical,
    headComponents
