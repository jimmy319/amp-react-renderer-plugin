...
test('render a valid AMP page with amp-apester-media component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-apester-media'
  const canonical = 'https://amp-apester-media.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-apester-media',
    AppComponent,
    title,
    canonical,
    headComponents
