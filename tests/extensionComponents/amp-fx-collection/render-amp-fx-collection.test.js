...
test('render a valid AMP page with amp-fx-collection component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-fx-collection'
  const canonical = 'https://amp-fx-collection.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-fx-collection',
    AppComponent,
    title,
    canonical,
    headComponents
