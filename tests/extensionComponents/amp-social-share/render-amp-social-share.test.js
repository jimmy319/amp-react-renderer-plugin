...
test('render a valid AMP page with amp-social-share component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-social-share'
  const canonical = 'https://amp-social-share.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-social-share',
    AppComponent,
    title,
    canonical,
    headComponents
