...
test('render a valid AMP page with amp-riddle-quiz component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-riddle-quiz'
  const canonical = 'https://amp-riddle-quiz.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-riddle-quiz',
    AppComponent,
    title,
    canonical,
    headComponents
