...
test('render a valid AMP page with amp-lightbox component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-lightbox'
  const canonical = 'https://amp-lightbox.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-lightbox',
    AppComponent,
    title,
    canonical,
    headComponents
