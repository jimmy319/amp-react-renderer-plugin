...
test('render a valid AMP page with amp-facebook-like component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-facebook-like'
  const canonical = 'https://amp-facebook-like.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-facebook-like',
    AppComponent,
    title,
    canonical,
    headComponents
