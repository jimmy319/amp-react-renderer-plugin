...
test('render a valid AMP page with amp-accordion component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-accordion'
  const canonical = 'https://amp-accordion.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-accordion',
    AppComponent,
    title,
    canonical,
    headComponents
