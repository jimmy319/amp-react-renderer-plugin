...
test('render a valid AMP page with amp-analytics component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-analytics'
  const canonical = 'https://amp-analytics.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-analytics',
    AppComponent,
    title,
    canonical,
    headComponents
