...
test('render a valid AMP page with amp-addthis component', async () => {
  // arrangement
  const AppComponent = <Application />
  const title = 'case - amp-addthis'
  const canonical = 'https://amp-addthis.test.com.tw'
  const headComponents = [<meta key='test' name='format-detection' content='telephone=no' />]

  // action
  const htmlString = ampReactRenderer({
    entryName: 'amp-addthis',
    AppComponent,
    title,
    canonical,
    headComponents
